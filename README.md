...
I keep **cognitive authority over the work**.

I use AI as an execution layer, not as the source of the architecture, logic or decision-making process. I define what the system should do, how the pieces relate to each other, what constraints must hold, what information is allowed to exist and what constitutes a correct result. The AI executes within that design.

That means I do not treat generated code, documentation or architectural suggestions as authoritative simply because a model produced them. I compare the implementation against the logic I intended, inspect deviations and decide what is accepted, changed or rejected.

This becomes even more important when I am designing something that will itself be consumed by an LLM. With ordinary backend code, an implementation mistake normally produces a software defect. With prompts, tool descriptions, schemas or agent-facing contracts, a small ambiguity can change the model's behaviour while still looking technically valid. I therefore spend more effort defining semantics, boundaries, allowed interpretations and response shapes before implementation.

My usual sequence is:

**design → AI-assisted execution → audit against the design → correction → validation**

The model accelerates execution. It does not own the reasoning process.

### When it went wrong

A concrete example happened during this assignment.

While turning my design into documentation and code, Claude introduced concepts that I had not defined or approved. They were plausible additions, which made the problem more dangerous: the result could look coherent while no longer representing the system I had designed.

I detected it by comparing the implementation against the decisions and semantic model I had already established. I removed or corrected those additions manually and tightened the way I constrained subsequent AI-assisted work.

The lesson for me is not simply to “review AI output”. It is to maintain a clear source of authority for the system before asking a model to implement it. This matters especially when the resulting artefact will itself guide another model, because an invented concept can propagate from implementation into tool behaviour and eventually into customer-facing decisions.

### In the room: if a client challenges a technical decision

If a client challenges a technical decision, I first try to understand the requirement behind the alternative they are asking for.

I explain the decision in terms of consequences rather than defending the implementation itself: what problem it solves, what guarantees it gives us, what we would gain by changing it and what we would lose.

I hold my position when the requested change would break an important requirement, remove a necessary boundary or create a failure mode that the current design deliberately prevents. I would not defend a decision simply because it was mine.

I change it when the client provides new information, a business constraint I did not have, evidence that one of my assumptions was wrong, or an alternative that satisfies the same requirements with a better trade-off.

The goal is not to win the technical argument. It is to make the reasoning explicit enough that we can make the right decision together.

---

## Scope and time

I spent approximately **five working days** on the assignment.

**3.5 working days were spent on design. The design was entirely mine, not generated by AI:** understanding the source data, defining the semantic model, deciding the responsibility boundaries between the agent and the Catalog Service, designing the tool contracts and response semantics, defining selection behaviour, failure handling, conversational state and orchestration.

The remaining **1.5 working days** were spent on execution and verification: auditing the AI-assisted implementation against my design, correcting deviations, configuring and validating the indigo.ai application, deployment and integration work, and preparing and recording the final demonstration video.

### Deliberately left out: MCP
