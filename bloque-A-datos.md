...
| **Nada de lo anterior** · solo `src/`, `tests/`, `Dockerfile`, `fly.toml`, el propio workflow | **No se ejecuta** | **No se ejecuta** |

**«Completo, siempre» dice cómo se recalculan las relaciones cuando se recalculan, no que se recalculen en cada commit.** Son dos preguntas distintas y confundirlas sale caro:

| Pregunta | Respuesta |
|---|---|
| ¿Puede `relate.py` recalcular solo una parte? | **No, nunca.** Una relación necesita el catálogo entero |
| ¿Tiene que ejecutarse `relate.py` en cada commit? | **No.** Solo cuando cambia el catálogo o el criterio |

Un commit que solo toca `src/api.py` no ha cambiado ni un producto ni una regla de clasificación. Volver a llamar al modelo ahí **tira una malla ya revisada y vuelve a tirar los dados** para obtener otra, a cambio de nada. Y como la respuesta del modelo no es determinista, la malla nueva puede ser distinta de la que se validó — que es exactamente lo que ocurrió: un commit sin productos nuevos produjo **56 relaciones y 89 productos participando**, contra los 32 que el catálogo sostiene.

**Quién decide, y con qué.** El workflow compara los ficheros del commit contra esos cuatro nombres. Sin historial con el que comparar, se reconstruye: es el primer despliegue de un repositorio, donde hay que construirla de todas formas.
...
| Versión | Cambio |
|---|---|
| v61 → v62 | **Se corrige el bloque de relaciones, que tenía tres fallos y ninguno lo delataba.** Una ejecución real produjo **56 relaciones, 89 productos participando y varias `equivalent`**, contra las **9 relaciones sobre 8 productos, 32 participantes y una sola `equivalent`** que el catálogo sostiene. **Uno:** el workflow ejecutaba `relate.py` en **cada commit**, así que un cambio en `src/` volvía a llamar al modelo y tiraba una malla ya revisada — A3.6 gana la fila que faltaba y la distinción entre *«completo cuando se recalcula»* y *«se recalcula en cada commit»*, que no son lo mismo. **Dos:** `normalize_relations` ordenaba la pareja y guardaba `pairs_with` bajo el identificador menor, aplicándole la regla de `alternative_to`; A4.7 dice que se guarda **del accesorio hacia el producto principal**, y en este catálogo eso **invertía las doce aristas**. **Tres:** descartaba en silencio identificadores inexistentes y autorreferencias, y convertía un `relation_type` inválido en `same_function` — es decir, reparaba justo lo que la puerta existe para detectar, garantizando que nunca lo viera. Ahora **cualquiera de esas respuestas para la construcción**. `prompts/relate.md` se reescribe: la dirección de `pairs_with`, y que **compartir `product_type` o `functional_family` no es motivo para escribir nada**, porque son los niveles que el servicio deriva en ejecución (B0.5). **Ni A4.7, ni A4.8, ni B0.5 cambian: ya decían lo correcto y el código no lo cumplía.** La puerta no se toca |
| v60 → v61 | **La clasificación se valida donde se produce, y el mapa del repositorio gana los dos ficheros del despliegue.** Un `Action` real falló en la puerta con `HL-014 → home_decor` y `JW-002` y `JW-007 → personal_style` en `functional_family`: los tres son valores reales **de `use_case`**, puestos en el campo equivocado. La puerta hizo su trabajo, pero demasiado tarde — el modelo ya había corrido sobre los 150 y el artefacto ya estaba sobrescrito. Ahora **`enrich.py` comprueba cada valor contra su vocabulario antes de escribirlo**, y cuando lo rechaza le dice al modelo **a qué vocabulario pertenece de verdad**, que es el error que comete: tres intentos, y si insiste la ejecución termina sin escribir nada. **La puerta no se toca y sigue rechazando exactamente lo mismo**: protegen cosas distintas, el artefacto una y la ejecución la otra. A3.9 gana **`.dockerignore` y `fly.toml`**, y el despliegue queda cerrado con **una sola Machine** (`--ha=false`), porque el contador del límite de B6.8 vive en la memoria del proceso y dos Machines llevarían dos contadores independientes: el límite real sería el doble del declarado sin que nada lo delatara. Queda escrito que `vocabularies.yaml` y `semantic_layer.json` viven **solo en `data/`**: las copias de la raíz se retiran, porque dos ficheros con el mismo nombre y distinto contenido clasificarían contra un vocabulario y validarían contra otro sin que el diff delatara nada. **Ninguna clasificación, ningún vocabulario y ninguna regla de selección cambian** |
...
**Dónde corta `is_standalone_gift`, dicho con todas las letras.** Corta **al recomendar** — en `find_products_by_criteria` y en `relation=alternative_to`, porque una alternativa sustituye al regalo y tiene que poder serlo. **No corta con `relation=pairs_with`**, que es la vía por la que un accesorio o un recambio llegan legítimamente como complemento: ahí un producto que no se sostiene solo como regalo **es exactamente lo que se está buscando**. Es lo que ya decía este corte —*"sigue disponible como complemento en el paso 9"*— y lo que exige B4.3, escrito ahora como regla y no como nota.
